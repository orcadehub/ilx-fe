      ...
      <Container className="mb-4">
        <div className="text-center mb-5">
          ...
          <p className="text-muted mx-auto" style={{ maxWidth: "700px" }}>
            Discover all the tools you need to elevate your influencer campaigns and measure success with elegance.
          </p>
          <div className="d-flex justify-content-center gap-3 mt-4 flex-wrap">
            ...
            <Button
              variant="primary"
              className="px-4 py-2 rounded-pill"
              style={{ backgroundColor: "#4a90e2", boxShadow: "0 6px 15px rgba(74,144,226,0.3)" }}
            >
              Schedule Demo
            </Button>
          </div>
        ...
        <Row className="g-4">
          {featuresData.map((feature, idx) => (
            <Col key={idx} xs={12} sm={6} lg={4}>
              <Card className="h-100 shadow-sm border-0" style={{ borderRadius: "20px" }}>
                <Card.Body>
                  <Card.Title className="fw-semibold mb-3" style={{ color: "#222" }}>
                    {feature.title}
                  </Card.Title>
                  <ListGroup variant="flush">
                    {feature.points.map((point, index) => (
                      <ListGroup.Item key={index} className="ps-0 border-0" style={{ color: "#555", fontSize: "0.95rem", background: "transparent" }}>
                        <span style={{ color: "#4a90e2", fontWeight: "bold", marginRight: "8px" }}>•</span>
                        {point}
                      </ListGroup.Item>
                    ))}
                  </ListGroup>
